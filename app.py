...
<style>
    ...
    .nav-item {
        ...
        border-bottom: 1px solid rgba(58, 80, 107, 0.35);
        text-decoration: none;
    }
    .nav-item:hover {
        background-color: rgba(78, 168, 222, 0.12);
        color: #ffffff;
...
def render_dashboard_layout(video_info, vid_comments, key_suffix=""):
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            # Normalize to 0-100% (5% like-to-view = 100%, 1% comment-to-view = 100%)
            l2v_score = round(min((l2v / 0.05) * 100, 100), 1)
            c2v_score = round(min((c2v / 0.01) * 100, 100), 1)
            overall_score = round((l2v_score + c2v_score) / 2, 1)

            bar_fig = go.Figure()
            metrics = [
                ("Overall Performance", overall_score, "#b7e335"),
                ("Comment-to-View",     c2v_score,     "#ffc91b"),
                ("Like-to-View",        l2v_score,     "#f78b30"),
            ]
            ...
            bar_fig.update_layout(
                xaxis=dict(range=[0, 100], ticksuffix='%', tickfont=dict(color='#e0e0e0'), gridcolor='rgba(255,255,255,0.1)', showgrid=True),
                yaxis=dict(tickfont=dict(color='#e0e0e0', size=14), automargin=True),
            ...
            st.plotly_chart(bar_fig, use_container_width=True, key=f"metrics_bar_{key_suffix}")
    ...
    if not vid_comments.empty:
        ...
        with col4:
            with st.container(border=True):
                ...
                with st.spinner("Extracting key topics..."):
                    all_comments_text = vid_comments['textOriginal'].dropna().tolist()
                    text_corpus = " ".join(all_comments_text)
                    ...
                    if text_corpus.strip():
                        ...
                        if keywords:
                            # Count how many comments mention each keyword
                            kw_rows = []
                            for kw, _ in keywords:
                                freq = sum(1 for c in all_comments_text if kw.lower() in c.lower())
                                kw_rows.append({'Topic / Keyword': kw, 'Mention Count': freq})
                            kw_df = pd.DataFrame(kw_rows)
                            kw_df = kw_df.sort_values('Mention Count', ascending=False).reset_index(drop=True)
        ...
        with st.container(border=True):
            ...
            selected_sentiment = st.selectbox(
                ...
                index=0,
                key=f"sentiment_filter_{key_suffix}"
            )
            if selected_sentiment != 'All':
                filtered_df = display_df[display_df['Sentiment'] == selected_sentiment]
            else:
                filtered_df = display_df
            
            st.caption(f"Showing {len(filtered_df)} of {len(display_df)} comments")
            st.dataframe(filtered_df, use_container_width=True, height=400, key=f"df_comments_{key_suffix}")
...
""", unsafe_allow_html=True)
    ...
    dash_active = 'active' if st.session_state.app_page == '📊 Dashboard' else ''
    hist_active = 'active' if st.session_state.app_page == '🕐 History' else ''
...
- A video upload costs 100 units."""
    ...
    st.markdown("**API Quota Used (Approx)**", help=help_text)
    quota_used = st.session_state.api_quota_used
    progress_val = min(quota_used / 10000.0, 1.0)
    st.progress(progress_val, text=f"{quota_used} / 10,000 credits")

if app_page == "📊 Dashboard":
    youtube_link = st.sidebar.text_input("Enter YouTube Video Link", placeholder="https://www.youtube.com/watch?v=...")
    
    # Auto-populate saved API key; update session state when user types a new one
    api_key_input = st.sidebar.text_input(
        "Enter YouTube API Key",
        value=st.session_state.saved_api_key,
        type="password",
        help="Your key is remembered for this session. Requires a free YouTube Data API v3 key from Google Cloud Console."
    )
